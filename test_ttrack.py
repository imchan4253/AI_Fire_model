...
if __name__ == "__main__":
    ...
    model_name = "p_h_b.onnx"
    aaa = glob.glob('/workspace/sam_person/_aibox/night_v/r_fire5.*')
    for filePath in aaa:
        with open(filePath, 'rb') as f:
             dataset=f.read()
             path = str(filePath.split('/')[5])
             PPath = "/workspace/sam_person/_aibox/track_hub_out/"+str(path.split('.')[0])+".txt"




             record = "/workspace/sam_person/_aibox/imchanyoung_out/"+str(path.split('.')[0])+"_out.mp4"
             ...
             frame_idx = 0
             Detection_count = 0
             tracker = MultiObjectTracker(dt=0.1, tracker_kwargs={'max_staleness': 15}, model_spec='constant_acceleration_and_static_box_size_2d', matching_fn_kwargs={'min_iou': 0.25})
             while True:
                 ...
                 if 0 in filtered_boxes:
                    ...
                    for track in tracks:
                        ...
                        Detection_count += 1 
                        print(ID, Detection_count)

                        cv2.rectangle(original, (int(track.box[0]), int(track.box[1])), (int(track.box[2]), int(track.box[3])), (0, 255, 0), 1)
                        cv2.putText(original, "fire" +str(id_counts), (int(track.box[0]+5), int(track.box[1]+40)), cv2.FONT_HERSHEY_SIMPLEX, 1, [0, 255, 0],2)

                        #with open(PPath, 'a') as wf:
                             #wf.write("fire"+str(id_counts)+" "+str(FF)+" "+str(newnew)+" "+str(FFF)+'\n')
